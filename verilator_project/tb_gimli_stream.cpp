...
#define G_FNAME_HASH_KAT "../data_tests/LWC_HASH_KAT_256.txt"
...
class Testbench {
    public:
        ...
        Testbench(long long stop_time_value=-1, int skipping_cycles=0) {
            ...
            dut->trace(tfp, 99);
            tfp->open("tb_gimli_stream.fst");
#else
            tfp = new VerilatedVcdC;
            dut->trace(tfp, 99);
            tfp->open("tb_gimli_stream.vcd");
#endif
#else
            stop_time_enable = 0;
            stop_time = 0;
...
int main(int argc, char **argv) {
    ...
    count = 0;
    #if G_SKIP_HASH_TEST == 0
        printf("Start of the hash test\n");
        hash_file = fopen(G_FNAME_HASH_KAT, "r");
        if(!hash_file != 0){
            printf("Could not open file %s\n", G_FNAME_HASH_KAT);
        }
        while(!feof(hash_file) && (count < G_MAXIMUM_NUMBER_OF_TESTS) && (test_error == 0)) {
            read_until_get_character(hash_file, '=');
            fscanf(hash_file, "%d", &count);
            printf("Test number %d\n", count);
            memset(test_input_message, 0, sizeof(char)*G_MAXIMUM_MESSAGE_SIZE_HASH);
            memset(true_output_hash, 0, sizeof(char)*G_MAXIMUM_HASH_SIZE);
            // Read Message
            read_until_get_character(hash_file, '=');
            read_ignore_character(hash_file, ' ', &temp_char1);
            pt_size = 0;
            while(temp_char1 != '\n') {
                temp_char2 = fgetc(hash_file);
                decode_hex_character(temp_char1, temp_char2, &test_input_message[pt_size]);
                temp_char1 = fgetc(hash_file);
                pt_size++;
            }
            // Read MD
            read_until_get_character(hash_file, '=');
            ...
            while(temp_char1 != '\n') {
                temp_char2 = fgetc(hash_file);
                decode_hex_character(temp_char1, temp_char2, &true_output_hash[ct_size]);
                temp_char1 = fgetc(hash_file);
                ct_size++;
            }
            for(j = 0; j < SKIPPING_TESTS_MAX; j++){
                tb->skipping_cycles = j;
                //printf("Skipping cycles = %d\n", j);
                memset(test_output_hash, 0, sizeof(char)*G_MAXIMUM_HASH_SIZE);
                // Perform the hash procedure
                tb->dut_hash(test_input_message, pt_size, test_output_hash, &ct_size);
                // Compare hash output
                test_verification = 1;
                if (memcmp(true_output_hash, test_output_hash, ct_size) == 0) {
                    test_error += 0;
                } else {
                    test_error += 1;
                    printf("Computed values during hashing do not match expected ones\n");
                    printf("Test number : %d\n", count);
                    printf("Expected hash: ");
                    for(i = 0; i < ct_size; i++){
                        printf("%02x", (true_output_hash[i] & 0x00FF));
                    }
                    printf("\n");
                    printf("Received hash: ");
                    for(i = 0; i < ct_size; i++){
                        printf("%02x", (test_output_hash[i] & 0x00FF));
                    }
                    printf("\n");
                }
            }
    ...
    test_error = 0;
        ...
        while(!feof(aead_file) && (count < G_MAXIMUM_NUMBER_OF_TESTS) && (test_error == 0)) {
            ...
            for(j = 0; j < SKIPPING_TESTS_MAX; j++){
                ...
                tb->dut_aead_enc(test_input_key_enc, key_size, test_input_nonce_enc, nonce_size, test_input_ad_enc, ad_size, test_input_pt_enc, pt_size, test_output_ct_enc, ct_size);
            ...
            for(j = 0; j < SKIPPING_TESTS_MAX; j++){
                ...
                tb->dut_aead_dec(test_input_key_dec, key_size, test_input_nonce_dec, nonce_size, test_input_ad_dec, ad_size, test_input_ct_dec, ct_size, test_output_pt_dec, pt_size);
